...
# Comprehensive Portuguese text beautifier
def beautify(text):
    text = clean(text)
    if not text:
        return ""
    
    # Fix standard uppercase terms to Title Case or clean Portuguese accents
    replacements = [
...
# Split into rows matching ;;;;52xxxxxx;
blocks = re.split(r'\n(?=;;;;52\d{6};)', raw_content)
...
for block in blocks:
    ...
    inep_raw = clean(parts[4])
    inep = re.sub(r'\D', '', inep_raw)
    
    if not inep or len(inep) != 8:
        continue
        
    if inep in seen_inep:
        continue
    seen_inep.add(inep)
    
    nome_orig = clean(parts[5])
    nome = beautify(nome_orig)
...
records.sort(key=lambda x: x["nome"])

print(f"Total de Escolas Processadas: {len(records)}")
for idx, r in enumerate(records, 1):
    print(f"{idx:02d}. [INEP: {r['codigo_inep']}] {r['nome']} | Gestor: {r['gestor_nome']} | End: {r['endereco']}")

# Save as JSON for importing in mockData.js
with open("c:/Users/rodri/.gemini/antigravity-ide/scratch/distribuicao-turmas-luziania/scratch/escolas_71_cleaned.json", "w", encoding="utf-8") as f:
